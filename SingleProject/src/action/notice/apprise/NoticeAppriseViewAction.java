...
public class NoticeAppriseViewAction implements Action {

	@Override
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		appriseDAO dao = appriseDAO.getinstance();
		appriseVO vo = dao.apprise_View(request.getParameter("idx"));
		...
		for(int x=0; x<cookies.length; x++){
			info = cookies[x];
			if(info.getName().equals("App_"+idx)){
				bool = true;
				break;
				// 쿠키 생성시 브레이크
			}
		...
		if(!bool){	// 쿠키가 존재하지 않으면
			// 조회수 증가 메소드
			dao.apprise_Readcnt(idx);
		
			info = new Cookie("App_"+idx, newValue);	// 쿠키 생성
		...
		request.getRequestDispatcher("notice/apprise_view.jsp").forward(request, response);
	}

...
public class GalleryFViewAction implements Action {

	@Override
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		fDAO dao = fDAO.getinstance();
		fVO vo = dao.f_View(request.getParameter("idx"));
		...
		if(!bool){	// 쿠키가 존재하지 않으면
			...
			info = new Cookie("F_"+idx, newValue);	// 쿠키 생성
		...
		request.getRequestDispatcher("gallery/gallery_f_view.jsp").forward(request, response);
	}

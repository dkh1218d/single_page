...
<div class="contain">
	...
	<div class="sub-view-contnet">
		...
		<br><br>
		<img class="contents_photo" src="gallery/gallery_f/${vo.photo}" alt="${vo.photo}">
	</div>
	
	<div class="sub-view-bottom">
		<c:if test="${!empty sessionScope.admin && sessionScope.admin.equals('T')}">
		<a href="SYKK?command=gallery_f_modify&idx=${vo.idx}&page=${page}" class="btn-modify">수정</a>&nbsp;&nbsp;
		<a href="javascript:avent()" class="btn-delete">삭제</a>&nbsp;&nbsp;
		</c:if>
		<a href="SYKK?command=gallery_f&page=${page}" class="btn-list">목록</a>&nbsp;&nbsp;
	</div>
...
function avent(){
	if(confirm("삭제하시겠습니까") == true) {
		location.href="SYKK?command=gallery_f_delete&idx=${vo.idx}";
	}else {
		return;
	}
}

...
<div class="contain">
	...
	<div class="sub-view-contnet">
		...
		<img class="contents_photo" src="notice/apprise/${vo.photo}" alt="${vo.photo}">
		</c:if>
	</div>
	
	<div class="sub-view-bottom">
		<c:if test="${!empty sessionScope.admin && sessionScope.admin.equals('T')}">
		<a href="SYKK?command=notice_apprise_modify&idx=${vo.idx}&page=${page}" class="btn-modify">수정</a>&nbsp;&nbsp;
		<a href="javascript:avent()" class="btn-delete">삭제</a>&nbsp;&nbsp;
		</c:if>
		<a href="SYKK?command=notice_apprise&page=${page}" class="btn-list">목록</a>&nbsp;&nbsp;
	</div>
...
function avent(){
	if(confirm("삭제하시겠습니까") == true) {
		location.href="SYKK?command=notice_apprise_delete&idx=${vo.idx}";
	}else {
		return;
	}
}
